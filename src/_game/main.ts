...
import { CubeTexture, Group, Scene, Texture } from 'three';
...
import { Landscape, createLandscape } from '@fdd/map-maker/landscape.ts';
...
export async function createGame(config: GameConfig): Promise<Game> {
    ...
    const landscape = await createLandscape(config.mapId, sdk);
