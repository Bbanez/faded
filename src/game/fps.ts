...
import { FunctionBuilder } from './math/function-builder.ts';
...
export class FPS {
    ...
    fpsToColor = FunctionBuilder.linear2D([
        [0, 0xff0000],
        [60, 0xffff00],
    ]);
    ...
    constructor() {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            el.style.width = `3px`;
            el.style.bottom = '0px';
            el.style.backgroundColor = `#${this.fpsToColor(i).toString(16)}`;
        ...
        const interval = setInterval(() => {
            ...
            for (let i = 0; i < this.data.length; i++) {
                ...
                let color = this.fpsToColor(this.data[i]).toString(16);
                if (color.length < 6) {
