...
import type { Storage } from '@banez/storage/types';
import type { Socket } from 'socket.io-client';
import type { SocketEvent, Store } from './types';
...
export interface SocketHandlerConfig {
  ...
  storage: Storage;
  cache: Store;
  refreshAccess(force?: boolean): Promise<boolean>;
...
export class SocketHandler {
  ...
  constructor(private readonly config: SocketHandlerConfig) {
  ...
  async connect(): Promise<void> {
    if (!this.isConnected) {
      ...
      return await new Promise<void>((resolve, reject) => {
        try {
          const token = this.config.storage.get<string>('at');
          if (!token) {
            this.isConnected = false;
            reject('You need to login to access socket.');
            return;
          }
          this.socket = io(this.config.origin, {
