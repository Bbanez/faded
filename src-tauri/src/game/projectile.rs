...
use crate::game::collision::collision_with_point;
...
pub struct Projectile {
    pub id: String,
    pub source_type: ProjectileSourceType,
...
impl Projectile {
    pub fn new(
        source_type: ProjectileSourceType,
        source_id: String,
        position: Point,
        target: Point,
        size: Size,
...
pub fn find_all_in_area(projectiles: &Vec<Projectile>, position: Point, size: Size) -> Vec<Projectile> {
    let mut output: Vec<Projectile> = vec![];
    for projectile in projectiles {
        let p_position = projectile.bounding_box.get_position();
        if collision_with_point(
            (position.y, position.x + size.width, position.y + size.height, position.x),
            (p_position.x, p_position.y),
        ) {
            output.push(projectile.clone())
        }
    }
    output
}
...
#[tauri::command]
pub fn projectile_find_all_in_area(state: tauri::State<GameState>, position: Point, size: Size) -> Vec<Projectile> {
    let state_guard = state.0.lock().unwrap();
    find_all_in_area(&state_guard.projectiles, position, size)
}
