...
function distance_between_nodes(start: [number, number], end: [number, number]): number {
...
function set_node_params(start: LocalNode, end: LocalNode, node: LocalNode): LocalNode {
    const n = node.clone();
...
function is_in_set(node: LocalNode, nodes: LocalNode[]): [LocalNode, number] | null {
    for (let i = 0; i < nodes.length; i++) {
        if (
...
function get_node_at_position(position: [number, number], nodes: LocalNode[]): LocalNode | null {
    for (let i = 0; i < nodes.length; i++) {
        const node = nodes[i];
