...
import { FddLayoutEntryMeta, FddMapEntryMeta } from '../../types/bcms';
    ...
    onMounted(() => {
      ...
      const gameToMapSpace = [
        FunctionBuilder.linear2D([
          [0, 0],
          [mapInfo.value.width, 256],
        ]),
        FunctionBuilder.linear2D([
          [0, 0],
          [mapInfo.value.height, 256],
        ]),
      ];
      unsub.push(
          ...
          if (refreshAt < cTime) {
            if (props.game.player && data.value.player) {
              const playerPosition: [number, number] = [
                gameToMapSpace[0](props.game.player.go.boundingBox.position.x),
                gameToMapSpace[1](props.game.player.go.boundingBox.position.y),
              ];
              if (
                playerPosition[0] !== data.value.player.position[0] ||
                playerPosition[1] !== data.value.player.position[1]
              ) {
                refreshAt = cTime + 100;
      ...
      <div class={`fixed top-4 right-4`}>
        ...
        <div
          ...
          style={`top: ${(28 + data.value.player.position[0]).toFixed(
            0,
          )}px; left: ${(20 + data.value.player.position[1]).toFixed(0)}px;`}
        />
        <div
          class={`relative`}
